...
async function assertBackendIsAvailable(request: APIRequestContext) {
  const response = await request.get(`${backendBaseUrl}/api/v1/system/ping`, {
    timeout: 5_000,
  });

  expect(
      response.ok(),
      `Backend is not available at ${backendBaseUrl}. Status: ${response.status()}`,
  ).toBeTruthy();
}
...
test.describe("Authenticated student dashboard scope", () => {
  ...
  test("dashboard uses the authenticated guardian primary student", async ({ page, request }) => {
    await assertBackendIsAvailable(request);
